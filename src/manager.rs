...
async fn read_config_snapshot() -> Result<ExtensionManagerV2ConfigSnapshot, String> {
    ...
    let host_api_base_url = if let Some(core_cfg) = yh_config_infra::core_crate_config::get_core_config() {
        ...
        format!("http://{}:{}/api/v1/plugin-host", host, server.get_main_port())
    } else {
...
impl PluginRuntimeManagerV2 {
    ...
    pub fn get_runtime_handle(&self, plugin_id: &str) -> Option<crate::runtime::RuntimeHandle> {
        self.active_runtimes.get(plugin_id).map(|entry| entry.clone())
    }

    pub fn list_runtime_handles(&self) -> Vec<crate::runtime::RuntimeHandle> {
        self.active_runtimes.iter().map(|entry| entry.clone()).collect()
    }
    ...
    pub fn remove_runtime_handle(&self, plugin_id: &str) -> Option<crate::runtime::RuntimeHandle> {
        self.active_runtimes.remove(plugin_id).map(|(_, handle)| handle)
    }
            ...
            if config.enabled {
                for path in [
                    ...
                    tokio::fs::create_dir_all(path)
                        .await
                        .map_err(|e| format!("failed to create plugin runtime dir '{}': {}", path, e))?;
                }
